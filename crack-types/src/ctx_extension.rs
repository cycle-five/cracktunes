// 
use crate::db::{MetadataMsg, PlayLog, User, UserVote};
use crate::guild::{operations::GuildSettingsOperations, settings::GuildSettings};
use crate::require_guild;
use crate::TrackReadyData;
use crate::{
    http_utils, http_utils::SendMessageParams, messaging::message::CrackedMessage,
    utils::OptionTryUnwrap, CrackedError, CrackedResult, Data, Error, MessageOrReplyHandle,
    NewAuxMetadata,
};
...
impl<'ctx> ContextExt<'ctx> for crate::Context<'ctx> {
    ...
    async fn check_and_record_vote(self) -> Result<bool, CrackedError> {
        ...
        // )
        // .await?;
        let has_voted_db = UserVote::has_voted_recently_topgg(i64::from(user_id), &pool).await?;
        let record_vote = has_voted && !has_voted_db;

        if record_vote {
            let username = self.author().name.to_string();
            User::insert_or_update_user(&pool, i64::from(user_id), username).await?;
            UserVote::insert_user_vote(&pool, i64::from(user_id), "top.gg".to_string()).await?;
        }
