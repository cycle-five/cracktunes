...
use crack_type::Error;

/// Default public key for testing purposes.
pub(crate) const DEFAULT_PUB_KEY: &str =
    "67c6bd767ca099e79efac9fcce4d2022a63bf7dea780e7f3d813f694c1597089";
...
fn handle_command(interaction: CommandInteraction) -> CreateInteractionResponse<'static> {
    CreateInteractionResponse::Message(CreateInteractionResponseMessage::new().content(format!(
        "Interactions endpoint online! Hello, <@{}>",
        interaction.user.id
    )))
}

/// Handles an incoming interaction request.
/// # Errors
/// Returns an error if the request body cannot be read or if the request fails cryptographic
/// verification.
fn handle_request(
    ...
    request.respond(
        tiny_http::Response::from_data(serde_json::to_vec(&response)?).with_header(
...
fn main() -> Result<(), Error> {
    ...
    let key = std::env::var("DISCORD_PUBLIC_KEY").unwrap_or_else(|_| DEFAULT_PUB_KEY.to_string());
